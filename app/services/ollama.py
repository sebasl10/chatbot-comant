...
async def call_ollama(prompt: str, system: str = None, stream: bool = False) -> str:
    payload = {"model": settings.model_ia, "prompt": prompt, "stream": False}
    if system:
...
async def stream_ollama(prompt: str, system: str = None):
    payload = {"model": settings.model_ia, "prompt": prompt, "stream": True}
    if system:
        payload["system"] = system
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        async with client.stream("POST", settings.ollama_url, json=payload) as res:
            ...
            async for line in (
                res.aiter_lines()
            ):  # httpx's method allows reading the response line by line as it is received.
                if line:
                    chunk = json.loads(line)
                    yield chunk.get(
                        "response", ""
                    )  # yield is used to send data gradually (streaming).
