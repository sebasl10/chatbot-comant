...
async def main():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, summary, description FROM ticket WHERE ticket.type IN ('Bug', 'Dev', 'Suggestion', 'Requête', 'Documentation')"
        )
        ...
        for ticket in tickets:
            ...
            current_batch["metadatas"].append(
                {"ticket_id": ticket_id, "source": "database_migration"}
            )
            ...
            if len(current_batch["ids"]) >= batch_size:
                ...
                print(
                    f"✅ Batch de {len(current_batch['ids'])} tickets ajouté - Total: {migrated_count}"
                )
